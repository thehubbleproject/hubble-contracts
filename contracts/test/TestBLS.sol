...
contract TestBLS {
  ...
  function hashToPointGasCost(bytes calldata data) external returns (uint256 p) {
    uint256 g = gasleft();
  ...
  function isOnCurveG2CompressedGasCost(uint256[2] calldata point) external returns (uint256) {
    uint256 g = gasleft();
    BLS.isOnCurveG2(point);
    return g - gasleft();
  }

  function isOnCurveG2GasCost(uint256[4] calldata point) external returns (uint256) {
    uint256 g = gasleft();
    BLS.isOnCurveG2(point);
    return g - gasleft();
  }
