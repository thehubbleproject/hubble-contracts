"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const ethers_1 = require("ethers");
const deploy_1 = require("../ts/deploy");
const argv = require("minimist")(process.argv.slice(2), {
    string: ["url", "root", "pubkeys"]
});
/*
    Note separate pubkeys with commas
    > npm run deploy -- --url localhost:8545 \
    --root 0x309976060df37ed6961ebd53027fe0c45d3cbbbdfc30a5039e86b2a7aa7fed6e \
    --pubkeys ,
*/
async function main() {
    const provider = new ethers_1.ethers.providers.JsonRpcProvider(argv.url);
    const signer = provider.getSigner();
    // const signer = new ethers.Wallet(
    //     "0xyourprivatekeyhere"
    // ).connect(provider);
    const parameters = {
        MAX_DEPTH: 20,
        MAX_DEPOSIT_SUBTREE_DEPTH: 1,
        STAKE_AMOUNT: "0.1",
        TIME_TO_FINALISE: 7 * 24 * 60 * 4,
        GENESIS_STATE_ROOT: argv.root
    };
    const allContracts = await deploy_1.deployAll(signer, parameters, true);
    Object.keys(allContracts).forEach((contract) => {
        console.log(contract, allContracts[contract].address);
    });
    const accountRegistry = allContracts.blsAccountRegistry;
    console.log("argv.pubkeys", argv.pubkeys.split(","));
    for (const pubkeyRaw of argv.pubkeys.split(",")) {
        const parsedPubkey = [
            pubkeyRaw.slice(64, 128),
            pubkeyRaw.slice(0, 64),
            pubkeyRaw.slice(192, 256),
            pubkeyRaw.slice(128, 192)
        ].map(_ => "0x" + _);
        console.log("Registering", parsedPubkey);
        const tx = await accountRegistry.register(parsedPubkey);
        tx.wait().then(_ => console.log("Done registering pubkey", pubkeyRaw.slice(0, 5)));
    }
}
main();
//# sourceMappingURL=deploy.js.map