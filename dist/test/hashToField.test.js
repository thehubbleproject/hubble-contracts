...
describe("Hash to Field", () => {
    ...
    it("expand message", () => {
        for (let i = 0; i < vectors.length; i++) {
            ...
            const out = hashToField_1.expandMsg(DST, msg, outLen);
            chai_1.assert.equal(utils_1.hexlify(out), v.expected);
        }
    });
    it("hash to point", () => {
        const expectedX = "0x09b6a2dec1f1b0747c73332e5147ecacde20767f28a9b68261713bed9a1d2432";
        const expectedY = "0x0cb70ff0b1bdb5d30006bd0cc03dc2c071dcff0daea886c9793f304c695c1bc6";
        const dst = "xxx";
        const msg = "0x616263";
        mcl.setDomain(dst);
        const p = mcl.hashToPoint(msg);
        const [x, y] = mcl.g1ToHex(p);
        chai_1.assert.equal(x, expectedX);
        chai_1.assert.equal(y, expectedY);
    });
});
//# sourceMappingURL=hashToField.test.js.map