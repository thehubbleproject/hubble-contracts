...
// UpFunc is the func signature for migrating.
type UpFunc func(*gorm.DB) error

// DownFunc is the func signature for rollbacking.
type DownFunc func(*gorm.DB) error
...
type Migration struct {
	...
	ID string
	// Up is a function that will br executed while running this migration.
	Up UpFunc
	// Down will be executed on rollback. Can be nil.
	Down DownFunc
}
...
// NewGormigrate returns a new Gormigrate.
func NewGormigrate(db *gorm.DB, options *Options, migrations []*Migration) *Gormigrate {
...
func (g *Gormigrate) runMigration(migration *Migration) error {
	...
	if !g.migrationDidRun(migration) {
		if err := migration.Up(g.tx); err != nil {
			return err
		}
